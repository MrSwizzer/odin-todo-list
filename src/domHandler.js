...
function createTodoElement(todoItem, onClick) {
    ...
    doneButton.disabled =
        currentTodoList.finishedTodos.includes(todoItem);
...
function displayTodoDetails(todoItem) {
    ...
    details.innerHTML = `
        <form id="todoDetailForm">
            ...
            <input type="text" id="detailTitle" value="${todoItem.title
        }" required /><br/>
            <label for="detailDescription">Description:</label>
            <textarea id="detailDescription" required>${todoItem.description
        }</textarea><br/>
            <label for="detailDueDate">Due Date:</label>
            <input type="date" id="detailDueDate" value="${todoItem.dueDate.toISOString().split('T')[0]
        }" required /><br/>
            <label for="detailDueTime">Due Time:</label>
            <input type="time" id="detailDueTime" value="${todoItem.dueDate
            .toTimeString()
            .slice(0, 5)}" /><br/>
            <label for="detailPriority">Priority:</label>
            <select id="detailPriority">
                <option value="low" ${todoItem.priority === 'low' ? 'selected' : ''
        }>Low</option>
                <option value="medium" ${todoItem.priority === 'medium' ? 'selected' : ''
        }>Medium</option>
                <option value="high" ${todoItem.priority === 'high' ? 'selected' : ''
        }>High</option>
            </select><br/>
...
function setupDetailForm() {
    ...
    form.onsubmit = (e) => {
        ...
        try {
            ...
            currentTodoItem.editDueDate(
                new Date(
                    `${dueDate.toDateString()} ${dueTime}`
                )
            );
    ...
    document.getElementById('closeDetailSectionButton').onclick = () => {
        document.querySelector('.detailSection').style.display =
            'none';
    };
...
document.getElementById('showFinishedTodosButton').addEventListener(
    'click',
    () => {
...
document.getElementById('addNewTodoListButton').addEventListener(
    'click',
    () => {
        document.getElementById('newTodoListForm').style.display =
            'block';
    }
);
...
document.getElementById('cancelTodoListButton').addEventListener(
    'click',
    () => {
        document.getElementById('newTodoListForm').style.display =
            'none';
...
document.getElementById('deleteCurrentTodoListButton').addEventListener(
    'click',
    () => {
        if (currentTodoList) {
            const index =
                currentTodoLists.indexOf(
                    currentTodoList
                );
            if (index !== -1) {
                currentTodoLists.splice(index, 1);
                currentTodoList =
                    currentTodoLists.length > 0
                        ? currentTodoLists[0]
                        : null;
                displayTodoLists(currentTodoLists);
            }
